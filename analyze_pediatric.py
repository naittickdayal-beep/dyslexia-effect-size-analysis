...
if len(subjects) != 35:
    ...
    raise ValueError(
        f"Expected 35 pediatric subjects, found {len(subjects)}. "
        f"Missing: {missing}"
    )

if n_dyslexia != 17:
    raise ValueError(
        f"Expected 17 dyslexic children, found {n_dyslexia}"
    )

if n_control != 18:
    raise ValueError(
        f"Expected 18 control children, found {n_control}"
    )
...
write_csv(
    "pediatric-statistical-results.csv",
    results
)

write_csv(
    "pediatric-subject-measurements.csv",
    subjects
)

write_csv(
    "pediatric-ratio-welch-test.csv",
    welch
)

write_csv(
    "pediatric-archive-audit.csv",
    audit
)
...
figure_path = (
    output_dir
    / "Figure1_Pediatric_Comparisons.png"
)
...
(
    output_dir
    / "pediatric-analysis-results.json"
).write_text(
